...
5. **Site size (1–5)** — how large the published site is: **1** = a single page, **3–4** = a focused/full multi-page review, **5** = 6–9 corpus-derived artifacts (see `reference/site-structure.md`). Default 3–4; let the corpus drive it (more papers + more distinct sub-themes → bigger). Confirm the size with the user. *Reading is always thorough regardless of site size* — every paper is read in full in Phase 2; for very large sets, cluster low-priority references into grouped reading agents (still full-text), but say so.
...
**Pick the site size (1–5) first** (per `reference/site-structure.md`) — it sets how many artifacts you build, from a single page (size 1) to 6–9 (size 5). Then write the topic site into `docs/<slug>/`, with thematic pages **derived from THIS corpus's own sub-topics, not a fixed template**, following **`reference/site-structure.md`** and **`reference/provenance-conventions.md`**. Use **`assets/page-template.html`** as the per-page skeleton.
...
- **It is a literature review of THIS corpus — not a verdict on the field.** This is the load-bearing rule. Every claim is scoped to the N papers brought to this build: write "across these N papers" / "in this corpus," **never** "the field is moving toward X," "the state of the art is Y," or anything that asserts where the world is heading. The corpus is a *sample* — often just what a search surfaced — not a census; it may be biased or incomplete (especially when sourced from `/arxiv-paper-finder`, where query phrasing shapes what was found). The Overview must state plainly how many sources were read, roughly when they were gathered, and that the review speaks only to them. The Synthesis page's bets/reference model are "what this corpus suggests," not forecasts. When you catch yourself generalizing past the corpus, stop and re-scope.
